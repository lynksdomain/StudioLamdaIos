...
//  Studio Lamda
//
//  Created by Antonio Flores on 1/2/21.
//
...
class SegmentedProgressBar: UIView {
    ...
    private var stopPoint = 0
    private var hasDoneLayout = false // hacky way to prevent layouting again
    private var currentAnimationIndex = 0
    
    init(numberOfSegments: Int,stopPoint: Int,duration: TimeInterval = 5.0) {
        self.duration = duration
        self.stopPoint = stopPoint
        super.init(frame: CGRect.zero)
        
        for n in 0..<numberOfSegments {
            let segment = Segment()
            if stopPoint >= n+1 {
